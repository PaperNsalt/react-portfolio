            ...
            <div key={rowIndex} className="row" ref={el => { rowRefs.current[rowIndex] = el; }}>
              {[...Array(7)].map((_, itemIndex) => {
                ...
                return <div key={itemIndex} className="row__item"><div className="row__item-inner" style={{ backgroundColor: '#111' }}>
                  {typeof content === 'string' && content.startsWith('http') ? <div className="row__item-img" style={{ backgroundImage: `url(${content})` }} /> : <div className="row__item-content">{content}</div>}
                </div></div>;
              })}
            </div>
          ))}
